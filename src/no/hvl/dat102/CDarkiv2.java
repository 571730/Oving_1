...
public class CDarkiv2 implements CDarkivADT{
    ...
    public void slettCD(int id) {
        if (antall == 0){
            ...
            for (int i = 0; node != null && !slettet; i++){
                if (node.getElement().getId() == id && i == 0){
                    start = node.getNeste();
                    antall--;
                    slettet = true;
                }
                else if (node.getElement().getId() == id){
            ...
            if (!slettet){
                System.out.println("\nFant ikke filen du ville slette på lageret\n");
            }
            else {
                System.out.println("\nSlettet platen med id " + id + " fra arkivet\n");
            }
        }
    ...
    public CD[] finnArtist(String delstreng) {
        ...
        LinearNode<CD> node = start;
        while (node != null){
            if (node.getElement().getNavn().toLowerCase().contains(delstreng.toLowerCase())){
                tempArkiv[teller] = node.getElement();
                teller++;
            }
            node = node.getNeste();
